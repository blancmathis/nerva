...
Codex Desktop then owns recording and transcription with the microphone selected on the Mac; neither audio nor resulting text returns through Codex Pad. If the bridge restarts while a gesture is held, the safe recovery is to stop dictation on the Mac; Codex Pad never fabricates a release. See [ADR 002](adr/002-cdp-adapter.md).
...
On the 22 July 2026 local baseline, static inspection and fixture coverage confirm the installed six-slot compatibility structure, the exact FAST/SPLIT/MIC/CODEX identities and dynamic versioned renderer discovery. The installed app-server schema exposes `model/list`, `skills/list`, metadata-only exact-thread read/resume, bounded `thread/turns/list`, and `thread/settings/update`. Setup generates the installed-version schema cache automatically, and runtime reports whether it matches instead of assuming an older probe remains valid. Home focus uses only the existing bounded session catalog. Capture Inbox adds no bridge command or protocol contract. Web Push unit and route tests cover private persistence, VAPID stability, endpoint/key rejection, exact authenticated registration, device revocation, event filtering, generic copy, pinned-completion grouping, safe deep links and terminal subscription cleanup.

The setup-provisioned private app-server control socket completed a real WebSocket initialize handshake. The latest bounded probe returned a seven-entry live model catalog, 54 cwd-scoped skills and 21 sessions without logging their private contents. A second live probe repeated metadata-only read, exact latest-turn resolution, skill listing and model listing for 12 consecutive active-task cycles with no disconnect. The owner also observed the previous atomic Dictation dispatch reach `0:00` and immediately release, which proves native delivery but not sustained capture.

The current Mac renderer accepted and displayed a generated PNG through the fixed composer attachment path without a submit, and the diagnostic attachment was removed afterward. The physical iPad `Send` gesture, corrected paired dictation, Apple Pencil/palm behavior and long iPadOS suspension still require the manual device checklist.

Local automated tests cover the typed native projection, the fixed composer expression and its no-submit invariant, pairing, Product State revision conflicts, 0–12 Home layouts, priority/status focus and pinned-attention ordering, direct long-press/drag ordering with post-drop click suppression, the absence of whole-page Session navigation after touch drags, explicit Home return through the product mark, Capture Inbox local persistence/schema migration/absence of assignment/exact Session reuse/offline reconnect/no-send/Pencil sketch, drawing, image normalization, Saved Drawings, ordered Review drafts, comparisons, registered-site compatibility, the Sites page, exact-session live-tab discovery, typed HTTP(S) navigation, global favorites, Site control/annotation and the Site QA Record flow through exact report delivery across Chromium/WebKit iPad landscape/portrait plus phone fixtures. Those checks plus the live Mac diagnostic prove local contracts, not complete physical iPad/Pencil behavior, the owner's live session catalog or every future Desktop version.

Full Desktop reciprocal-peer authority, a bounded multi-image app-server turn and sustained live native Dictation remain unproven. Live Site has an implemented bounded tab driver, but the real Codex Browser frame/control path and physical Pencil-to-attachment flow still need owner verification on iPad. The previous atomic Dictation gesture did reach the exact native task and immediately stop; the paired replacement is automated-test green but still needs physical confirmation. Tailscale pairing and installed-PWA credential reuse have worked on the owner's Mac/iPad, but Pencil/camera behavior plus the complete lifecycle matrix remain unproven. The repository therefore has a locally verified implementation with explicit compatibility gates, not a hardware-ready or complete end-to-end claim.

...
The adapter exposes fixed one-image and ordered-batch composer attachment operations. Inputs are limited to one canonical thread UUID, one to twelve uniquely named Nerva PNGs, 8 MiB per image and 24 MiB for the complete batch. The renderer expression revalidates the exact visible task, finds one unique native add-context control and its live React paste handler, and dispatches one `ClipboardEvent` containing every validated in-memory `File`. It waits for every matching remove-attachment control to appear and distinguishes no visible file from a partially visible batch. It contains no submit call, keyboard shortcut, arbitrary expression or app-server method. The advertised `composerAttachmentMaxImages` remains `1` until this exact Desktop primitive has been attested; Draw then exports one bounded atlas instead of attempting a batch.

Capture Inbox uses a separate fixed generic-file batch primitive. The protocol accepts one to four files, 8 MiB each and 16 MiB total, with safe unique display names, syntactically bounded MIME types, canonical padded base64 and one canonical target thread. The native expression repeats the exact visible-composer proof, builds the in-memory `File` objects, dispatches one paste event and confirms every unique remove-attachment label. No file bytes or retry marker are persisted by the bridge command, no partial remainder is completed automatically, and the expression contains no composer submission path.
...
| `POST /api/command` with `openBrowserTab` | Durable exact-task request to create a validated HTTP(S) page through Codex's native Browser host flow. The outcome is confirmed from the native task inventory; uncertain outcomes are not replayed. |
...
The Session `Sites` key opens one unified touch picker containing only the current task's proven HTTP(S) pages from the verified Codex Browser renderer. There is no visible linked/unlinked split. The native Browser snapshot's canonical owner route establishes task ownership; title, displayed URL, focus and timing never do. Choosing a row operates that exact opaque tab. Entering an address or choosing a favorite creates a new native page for the exact task and never silently replaces an existing page.

The bridge resolves the opaque tab ID against a fresh attested inventory before every frame or input operation. The PWA receives bounded JPEG frames and may send only the typed controls `tap`, `scroll`, bounded text insertion, four allowlisted keys, back, forward and reload. A separate durable command accepts one validated HTTP(S) address solely to ask Codex to create a new page for the exact task. Debugger addresses, CDP methods, JavaScript and raw protocol payloads never cross the bridge API. The full-screen Site workspace behaves as a live page until Pencil contact—or touch after enabling `Touch + Pencil`—freezes the current frame for simple ink annotation. It contains no filmstrip, blank-frame command, Photo/Files or Camera import. `Send` exports one annotated PNG through the existing exact native composer-attachment path and does not submit the Mac composer.
...
The generic reconciliation list retains at most 64 canonical pending command UUIDs in `localStorage`. A separate drawing-delivery store retains at most 64 immutable per-thread bindings: command/slot/thread identity, expected snapshot sequence, the fixed empty-instruction identity, and the SHA-256 identity of the serialized scene. IndexedDB separately retains the exact ordered PNG bytes, tiling manifest, board/checkpoint identity and scope for an unresolved Draw transfer. An explicit retry therefore uses the same bytes and command ID; a late success can create the correct `Sent` checkpoint after a reload.
...
The small `localStorage` recovery ledger stores no mutation body, PNG/base64, raw scene JSON, instruction or transcript text, credential, media, or replay instruction. Draw's media bytes live only in the origin-scoped IndexedDB package described above. Reload recovery first uses authenticated `GET` status checks; an explicit retry may reuse that immutable package and the same command ID. Recovery never auto-submits a mutation and fails closed if the target, digest binding or retained package no longer matches.

### `packages/drawing`

The drawing engine owns a serializable vector scene in world coordinates bounded to ±1,000,000 units. The physical canvas never exceeds the current viewport; a separate `{centerX, centerY, zoom}` camera projects visible world content. An adaptive grid is anchored in world space, an order-preserving spatial index culls off-screen elements, and history stores bounded reversible operations instead of scene snapshots. Pointer Events preserve coordinates, time, pressure, and tilt when supplied. Pen input uses pressure-aware `perfect-freehand`. Geometry tools and text remain editable until export.
...
The camera is navigated by two fingers in Pencil mode or the explicit Hand tool. Select performs z-order-aware hit testing across ordinary scene elements and rendered graph nodes; an empty-area drag creates a multi-item selection. A selected group can be moved from anywhere inside its bounds or uniformly resized with one 44-pixel handle. Scene replacements commit as one reversible operation, and mixed scene/graph transforms are represented as one UI history transaction. Eraser closures move with the selected ink so a transform cannot reveal previously erased marks. A contextual minimap appears during pan, zoom and Fit board, then disappears after inactivity; it can recenter the camera without changing content. Board persistence stores stable metadata and only changed element records, while camera-only autosaves avoid serializing the scene.

Export is bounded but spatial size is not a blocking condition. `Whole board` uses real content bounds; `Select area` uses a touch-resizable world rectangle. A legible region becomes one PNG up to 4096 × 4096. Larger regions become a self-describing visual package: `01-map` plus at most eleven rectangular 2048-detail tiles whose cores cover the selected world without gaps and whose render bounds add 12% symmetric overlap.

Deterministic `A1`/`B2` regions, external detail headers, neighbor lists, highlighted mini-maps and stable filenames make every image independently localizable. Each neighboring pair repeats the same `R-…` registration code and visual marker in both external gutters. Seam search prioritizes intact blocks/text/photos, then structured links, freehand ink and balanced empty space. If more regions are needed, detail scale is reduced uniformly. Every image remains at most 8 MiB, the batch remains at most 24 MiB and cumulative decoded pixels remain bounded; canvases are produced sequentially. Without exact multi-image composer attestation, the map receives roughly 40% of one 4096 atlas and the ordered details use the remaining area. See [ADR 003](adr/003-drawing-engine.md).

Collaborative diagrams add a validated structured layer above that engine. Version 2 accepts negative positions and the same wide world bound, with at most 256 blocks and 512 referenced links for one exact task. Version 1 documents migrate without moving content: their historical center remains `(720, 450)`. Auto-layout expands into available world space instead of compressing blocks into a page. A graph has no separate canvas or bounding frame: its nodes and edges are projected into the same world and camera as photos, shapes and Pencil elements. New nodes are centered in the current viewport. Select maps rendered node geometry back to its structured node identity, while ordinary Pencil elements remain stored separately. Export merges the current graph render and Pencil scene only for rendering.

A dirty structure is written with an optimistic expected revision before Keep or before any Send PNG is built; a stale write conflicts instead of replacing a newer revision. The exported map records the exact Diagram revision and a compact node/region and cross-region edge index. Known edges crossing tile seams receive stable matching continuation codes; freehand marks never receive inferred semantics. When the index does not fit legibly on the map, a dedicated structure-index PNG precedes the detail tiles.
...
Drawing export follows the tile or atlas plan above. The bridge validates every member before any composer mutation, canonicalizes images sequentially and rechecks the exact task at the native paste boundary. A complete batch uses one paste event. All expected filenames visible means confirmed; none visible keeps a manually retryable unknown result; a partial set requires resolution on the Mac and Nerva never completes it automatically. Review remains a separate app-server flow with its own 64-megapixel aggregate decode budget and atomic conversion rules.
...
Current live Site interaction operates the exact Codex Browser target chosen from the requested Session's scoped inventory. The bridge reads Codex Desktop's Browser snapshot, verifies the canonical owner route, resolves the exact webview identity and matches one CDP target through a read-only navigation-generation fingerprint. Duplicate URLs remain distinct; ambiguous target matches stay unavailable. It repeats this proof before every frame or gesture and accepts only the typed vocabulary documented in [ADR 007](adr/007-site-review.md). This preserves Mac-only page state without embedding the site or exposing its private origin to iPad storage.
...
The Session surface allows multiple selections for the next text-bearing Nerva payload. Assembly appends the English `Use the following skills for this task: …` sentence after every other instruction. A Drawing send is deliberately image-only and does not consume or inject selected skills; they remain armed for the next text-bearing action.

For compact `Send prompt`, the bridge first requires the exact live composer, the exact `ACT12` / `CODEX` / `composer.submit` binding, and every requested skill to remain enabled. A fixed renderer expression locally locates the one visible editor, checks only whether the validated suffix is already at its end, positions the caret and dispatches one text paste. Composer text never crosses into the bridge or PWA. Only after the suffix is confirmed does Nerva invoke the separately attested native submit binding. A definitive failure retains armed skills; a confirmed submit consumes them; an unknown result remains unresolved. Nerva still cannot report whether Codex Desktop chose Queue or Steer.
...
The generated PWA directory in a source checkout is mutable, but one bridge generation is not. Before listening, a production bridge verifies `app-meta.json` against its compiled build revision and API contract, copies the complete distribution into a private process-scoped snapshot, and serves only that copy. A later `npm run build` therefore cannot silently replace the client underneath an active bridge; a different PWA becomes reachable only with a new bridge generation. Missing or mismatched production metadata fails startup rather than weakening the exact-build mutation gate.

The root build orchestrator computes one source revision and supplies it to both application builds, then imports both outputs and rejects any revision or API-contract mismatch. Fixture and screenshot builds use a separate temporary output root that is removed when their server exits; they never write the production distribution.
...
Inside the renderer, the fixed primitive rechecks the exact composer, constructs the validated in-memory PNG `File` objects, dispatches them once to the live paste handler, and waits for the unique filename controls to appear. It never calls `turn/start`, `turn/steer`, a submit handler or a keyboard shortcut. It therefore works while the managed app-server is disconnected, provided the exact native composer remains live.

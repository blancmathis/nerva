...
Setup commit `b1ba22b` adds a read-only real-Mac proof: the current machine is classified `degraded`, not blocked, with Desktop `0.146.0-alpha.3.1`, standalone `0.145.0`, three writers, unverified ownership, and no native mutation. Commit `a2bd82a` was also validated from a fresh local clone after `npm ci` and Playwright browser installation: 932 unit tests and 11/11 probe-safety tests passed; the full E2E matrix reported 293 passes and 13 explicit exclusions with two workers and no retry; build and the 387.70 kB bundle budget passed; screenshot generation passed; the release audit inspected 439 files and 163 production dependencies; Context Room reported zero health issues; and the production dependency audit reported zero vulnerabilities. GitHub Actions run `30211288284` independently passed the required `Node.js 22` job for the same commit. Strict doctor remains red, as intended, because the native topology is not compatible and the bridge was not started or changed during this validation.

...
The app-server channel was proven live on an earlier Codex version, but that evidence is not carried forward. On 26 July 2026, the new read-only `npm run setup:check` reports **Ready with limited Codex controls** and `npm run doctor` remains red:

- three independent stdio app-server writers are present;
- Desktop ownership is not attested;
- the private managed socket responds with standalone Codex `0.145.0`;
- Codex Desktop bundles `codex-cli 0.146.0-alpha.3.1`.

Nerva now installs its private bridge, Serve route, and pairing in this degraded topology but does not bootstrap a daemon, stop a writer, set the Desktop daemon flag, create an attestation, restart Desktop, or simulate native capabilities. Pairing remains available. App-server-backed controls stay `Unavailable`, and doctor stays strict and nonzero until a compatible, positively attested topology exists.

The repository is public pre-alpha at [`blancmathis/nerva`](https://github.com/blancmathis/nerva), with no tag, GitHub Release, npm publication, or deployment. Baseline commit `0a95911` passed GitHub Actions [run 30204589636](https://github.com/blancmathis/nerva/actions/runs/30204589636): 927 unit tests, 11 safety tests, build, 293 E2E tests with 13 explicit profile exclusions, a 387.70 kB largest JavaScript chunk, a 438 files release audit, Context Room doctor, and dependency audits. The setup/documentation work described here must receive its own green CI run before its final SHA is recorded.
...
| Mac setup | Read-only human/JSON preflight; `Ready`, `Ready with limited Codex controls`, and `Blocked`; private Nerva state; exact Tailscale Serve route; bridge LaunchAgent; health-before-QR; pairing independent from native readiness | The current machine is limited because `0.145.0` standalone differs from Desktop `0.146.0-alpha.3.1`, three writers coexist, and ownership is unverified. Full native integration is not claimed. |
...
Setup commit `b1ba22b` adds a read-only real-Mac proof: the current machine is classified `degraded`, not blocked, with Desktop `0.146.0-alpha.3.1`, standalone `0.145.0`, three writers, unverified ownership, and no native mutation. On the combined setup/documentation candidate, 932 unit tests and 11/11 probe-safety tests passed; the full E2E matrix again reported 293 passes and 13 explicit exclusions; build and the 387.70 kB bundle budget passed; screenshot generation passed; the release audit inspected 439 files and 163 production dependencies; Context Room reported zero health issues; and the production dependency audit reported zero vulnerabilities. Strict doctor remains red, as intended, because the native topology is not compatible and the bridge was not started or changed during this validation. The final clean-clone and GitHub CI SHA will be recorded after the documentation commit is pushed.

Automated browser proof does not establish physical Apple Pencil behavior, iPadOS suspension, Web Push wake, or a complete native Codex topology. No `v0.1.0` tag may be created while doctor is red or the hardware checklist is incomplete.

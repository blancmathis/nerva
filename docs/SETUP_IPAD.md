...
- Open Skills and confirm the list corresponds to the exact session cwd rather than always showing zero. Arm multiple skills and confirm they remain armed after an image-only Drawing send. Then type a non-sensitive prompt on the Mac and tap `Send prompt`: the validated English Skills suffix must appear exactly once at the absolute end before the native submit, and a definitive rejection must leave the skills armed.
- From that exact Session, open Capture Inbox, select one to four small non-sensitive file-only captures and tap `Attach to composer`. Confirm every file appears in that exact Mac composer, the originals remain in Capture Inbox, no text is added, and no Queue/Steer/submit occurs. Repeat with Tailscale disconnected and confirm the action remains unavailable with no reconnect replay.
...
- Prepare text by typing on the Mac or using iPad Dictation, optionally arm Skills, then tap the compact **Send prompt** control. Confirm only the exact selected task's native composer is submitted, the validated Skills suffix appears once at the absolute end when armed, and Codex Desktop follows its own Queue/Steer setting; the iPad must not invent a delivery state.
...
The bridge reads Codex Desktop's native Browser snapshot and accepts a page only when its canonical owner route matches the requested task. It never infers task ownership from a page title, displayed URL, project, recency or position. Duplicate URLs remain separate; an ambiguous webview-to-target match stays unavailable. Choosing one opaque row operates that exact page, and before every frame or gesture the bridge proves that the ID still belongs to the task. Entering a validated HTTP(S) address asks Codex to create a new native page for that task. The iPad never receives query strings, fragments, credentials, debugger URLs, raw CDP or JavaScript execution.

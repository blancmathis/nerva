...
The last local doctor run on 21 July 2026 observed Codex Desktop `26.715.70719` build `5650`, bundle ID `com.openai.codex`, and bundled `codex 0.145.0-alpha.27`. This is an observation, not a hardcoded support promise. The Capability Center and doctor must report the currently installed versions and schema state after every update.

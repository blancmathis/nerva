...
| Component | Observed version/state on 7 September 2026 | Proof level |
| --- | --- | --- |
| Mac | macOS `26.5.1`, Apple Silicon `arm64` | Observed locally |
| Codex Desktop application | `/Applications/ChatGPT.app`, bundle ID `com.openai.codex`, version `26.901.51231`, build `8109` | Observed locally by doctor on 7 September 2026 |
| Desktop-bundled Codex | `0.153.4` | Exact binary fingerprint and regenerated cache of 416 schema files verified; the managed daemon now responds |
| OpenAI-managed standalone CLI | `0.153.4` at `~/.codex/packages/standalone/current/codex` | Separate fingerprinted cache of 416 schemas generated; live managed protocol probe passes |
...
| Native live slot values | Desktop CDP is unavailable; native Micro adapter is degraded | Current doctor warning; commands fail closed |
| Native composer image attachment | Historical diagnostic proof exists against `26.715.61943` | Not re-proven against the installed `26.901.51231`; physical device tap still pending |
| App-server writers | Independent stdio writers observed | Diagnostic warning only while they are not peers of the private managed socket; no external process was stopped |
| Managed control socket | Private managed control socket responds | Sessions, models, skills and usage are available; mutations still require Desktop ownership |
...
| Installed Nerva bridge | Loopback-only on `127.0.0.1:8787`; successful health responses report native Codex stale/offline | Mac-side health and visible pairing screen verified; current native control is not proven |
...
| Tailscale and pairing | Tailscale online, private HTTPS route and same-origin WSS upgrade verified; unauthenticated socket closes with `4401`; Funnel disabled | Current Mac-side route proof. Historical physical pairing is separate from a current phone/tablet acceptance run |
...
| Historical automated checkout | Commit `dda917b`: 941 unit tests plus 11/11 probe-safety tests, build, 387.70 kB largest JavaScript chunk, 293 E2E passes with 13 explicit profile skips, 441-file release audit, screenshots, Context Room doctor and production audit. GitHub Actions run `30219005029` passed. | Historical clean-clone and hosted-runner proof |
| Published hardening snapshot | Commit `bb45368`: 1,030 unit tests plus 19/19 probe-safety tests, build, 402.15 kB maximum observed largest JavaScript chunk, 343 E2E passes with 17 explicit profile exclusions, six-profile production bridge harness, 463-file working-tree release audit, screenshots, docs, Context Room, and dependency audits. The exact non-ignored source snapshot reproduced the full gates from a clean local clone with a 452-file release audit. GitHub Actions run `30661070563` passed the required `Node.js 22` job and the read-only macOS setup job. | Local, clean-clone, and hosted-CI proof; the explicitly restarted LaunchAgent serves the immutable exact-build snapshot and passed raw-protocol plus live-origin WebKit PWA simulations; not physical-device proof |
| Current dependency refresh | Fastify `5.12.3`, fast-uri `3.1.7` / `4.1.4`, process-warning `5.1.0`; refreshed license inventory | See the [dated validation](product/CURRENT_STATE.md) for current local gates; no new hosted CI or hardware proof |
| Setup preflight | The managed transport responds, but native integration remains limited by unverified Desktop ownership and unavailable Desktop CDP. Both exact schema caches are present; private HTTPS/WSS has been verified from the Mac. | See the [dated implementation state](product/CURRENT_STATE.md) for the current preflight result; no native capability was simulated |

The Home catalog reads indexed interactive sessions through Codex's managed `thread/list` API with `useStateDbOnly: true` on every page. It follows opaque cursors, preserves the daemon's default interactive-source filter, removes duplicate UUIDs, and remains bounded to five pages / 500 sessions. This is the daemon's indexed catalog: unindexed legacy rollouts are not discovered or repaired during a foreground refresh. Nerva does not open Codex's database or rollout files directly. On 7 September, the same 100-item request timed out after 10 seconds with the default scan-and-repair behavior but returned 77 indexed sessions with an exhausted cursor in 17 ms; that observation does not prove coverage of unindexed history.
...
| Native Codex Voice | Exact active task, one enabled native `Start voice chat` control observed in the current renderer, fresh shared Desktop ownership, and write-adjacent task/control revalidation. Nerva clicks only that fixed native control; it never guesses eligibility from task age, substitutes Dictation, requests the iPad microphone, or implements a separate Realtime client. |
